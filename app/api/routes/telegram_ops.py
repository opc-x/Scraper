...
class ClassifyRequest(BaseModel):
    ...
    account_id: int | None = None
    limit: int = 100
    messages: list[MsgInput] | None = None  # 前端直传，跳过 Telegram fetch


async def _llm_batch(api_key: str, payload: list) -> list:
    """单批 LLM 调用，返回 items 列表"""
    import asyncio
    try:
...
async def classify_messages(req: ClassifyRequest):
    ...
    # ── 1. 消息来源：前端直传 > Telegram fetch（省去 6s+ 拉取）──
    if req.messages is not None:
        raw = [{"id": m.id, "text": m.text} for m in req.messages]
    else:
        client = await _require_client(req.account_id)
        entity = await client.get_entity(_resolve_entity_id(req.target))
        raw = []
        async for msg in client.iter_messages(entity, limit=min(req.limit, 500)):
            raw.append({"id": msg.id, "text": msg.text or ""})
    ...
    if api_key and passable:
        BATCH = 100
        batches = [
            [{"id": m["id"], "text": m["text"][:300]} for m in passable[i:i+BATCH]]
            for i in range(0, len(passable), BATCH)
        ]
        results = await asyncio.gather(*[_llm_batch(api_key, b) for b in batches])
    ...
    # ── 6. 合并结果（只返回 id/keep/tag/reason，前端自己有完整消息）──
    result = []
    for m in raw:
        if m["id"] in cached:
            c = cached[m["id"]]
            result.append({"id": m["id"], "keep": c["keep"], "tag": c["tag"], "reason": c["reason"]})
        elif m["id"] in classifications:
            cls = classifications[m["id"]]
            result.append({"id": m["id"], "keep": cls.get("keep", True),
                           "tag": cls.get("tag", "其他"), "reason": cls.get("reason", "")})
        elif rule_keep(m):
            result.append({"id": m["id"], "keep": True, "tag": "未分类", "reason": "规则通过"})
        else:
            result.append({"id": m["id"], "keep": False, "tag": "垃圾", "reason": "规则过滤"})
    ...
    return {
        ...
        "kept": kept,
        "classifications": result,  # 只有分类结果，不含消息体
    }
